...
public class ChatAdapter extends ArrayAdapter<Chat> {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        // the author.
        // OVERDRAW FIX - only set the background the color when an avatar isnt present
        if (chat.getAuthor().getAvatarId() == 0) {
            // No avatar image available, so image->transparent and background->color
            ...
            // Avatar image available, so image->transparent and background->color
